	...
	public function view(){
		...
		if ($this->input->hasPostReques() && $this->input->hasPost('action')) {
			...
			if ($this->input->post('action') == 'like') {
				$u = $dataVotes->addLikeDislike($albumId,1,0);
				if ($u != false && ($u > 0)) {
					$this->AddNoticeMessage('Voted !');
				} else {
					$this->AddErrorMessage('You already voted!');
				}
			}
			 ...
			 * ADD DISLIKE
			 */
			if ($this->input->post('action') == 'dislike') {
				$u = $dataVotes->addLikeDislike($albumId,0,1);
				if ($u != false && ($u > 0)) {
					$this->AddNoticeMessage('Voted !');
				} else {
					$this->AddErrorMessage('You already voted!');
		...
		$this->view->appendToLayout('body','album');
